...
label lb_enc_redhood:
    ...
    if persistent.redhood_done: #проверяем не было ли уже этого энкаунтера за всё время игры
        jump lb_encounter_plains #если уже был хотя бы раз, игнорируем и переходим к обычным встречам
    $ persistent.redhood_done = True #отмечаем что энкаунтер с красной шапочкой произошел
    ...
    show expression 'img/bg/forest/1.png' as bg    
    ...
    nvl clear
    show expression 'img/scene/wolf_sex.png' as bg
    pause (50.0)
...
label lb_enc_granary:
    show expression 'img/bg/plain/granary.png' as bg    
...
label lb_patrool_plains:
    ...
    $ chances = show_chances(game.foe)
    call lb_fight from _call_lb_fight_12
